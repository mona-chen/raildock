...
# Strip control chars and surrounding whitespace from an env var value so the
# generated .env file remains bash-sourceable. Used for any value that came
# from outside (paste, file read, prior .env) rather than generated locally.
sanitize_env_value() {
  local val="$1"
  # Remove CR, LF, TAB, NUL and any other control char (except space); trim
  # surrounding whitespace. Without this, a trailing newline would make
  # bash fail with "unexpected character '/' in variable name" errors when
  # `source`-ing the .env file.
  printf '%s' "$val" | tr -d '\000-\037\177' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//'
}
...
create_env() {
  ...
  if [ -f "$INSTALL_DIR/backend/config/master.key" ]; then
    master_key=$(tr -d '[:space:]\000-\010\013-\037\177' < "$INSTALL_DIR/backend/config/master.key")
  ...
  public_url=$(sanitize_env_value "${RAILDOCK_PUBLIC_URL:-$(get_public_url)}")
  public_host=$(sanitize_env_value "${RAILDOCK_PUBLIC_HOST:-$(get_public_host "$public_url")}")
  dokku_host=$(sanitize_env_value "${DOKKU_HOST:-$(detect_dokku_host)}")
