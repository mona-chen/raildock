    ...
    <div className="flex items-center gap-2 mb-6">
      <button
        ...
        className={`flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full transition-all ${
          step === 'bootstrap' ? 'bg-rail-purple/20 text-rail-purple' : 'text-[#8a8a99] hover:text-white'
        }`}
      >
      ...
      <button
        ...
        className={`flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full transition-all ${
          step === 'connect' ? 'bg-rail-purple/20 text-rail-purple' : 'text-[#8a8a99] hover:text-white'
        }`}
      >
      ...
      <div className="bg-[#18181B] border border-[rgba(255,255,255,0.08)] rounded-2xl p-6 w-full max-w-[520px] max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#8a8a99]">Connect a remote host to RailDock</p>
          </div>
        </div>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-[10px] text-[#8a8a99]">Organization public key</label>
                    {bootstrap?.publicKey && (
                      <button
                  ...
                  <p className="text-[10px] text-[#6b6b7b] mt-1">
                    Add this to <code className="text-[#8a8a99]">~/.ssh/authorized_keys</code> for the admin user and the deployment user (e.g. <code className="text-[#8a8a99]">/home/dokku/.ssh/authorized_keys</code>).
                  </p>
                </div>

                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-[10px] text-[#8a8a99]">Bootstrap command</label>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <label htmlFor="server-name" className="text-[11px] text-[#8a8a99] block mb-1.5">Server Name</label>
                <input
                  id="server-name"
                  value={name}
              ...
              <div className="col-span-2 sm:col-span-1">
                <label htmlFor="server-host" className="text-[11px] text-[#8a8a99] block mb-1.5">Host / IP</label>
                <input
                  id="server-host"
                  value={host}
              ...
              <div className="col-span-2 sm:col-span-1">
                <label htmlFor="admin-user" className="text-[11px] text-[#8a8a99] block mb-1.5">Admin User</label>
                <input
                  id="admin-user"
                  value={adminUser}
                  ...
                  placeholder="root"
                />
                <p className="text-[10px] text-[#6b6b7b] mt-1">Must have passwordless sudo or root access.</p>
              </div>
              <div className="col-span-2 sm:col-span-1">
                <label htmlFor="ssh-user" className="text-[11px] text-[#8a8a99] block mb-1.5">Deployment User</label>
                <input
                  id="ssh-user"
                  value={sshUser}
              ...
              <div className="col-span-2 sm:col-span-1">
                <label htmlFor="proxy-mode-connect" className="text-[11px] text-[#8a8a99] block mb-1.5">Proxy Mode</label>
                <Select value={proxyMode} onValueChange={(value) => setProxyMode(value as 'managed' | 'external')}>
              ...
              {proxyMode === 'external' && (
                <div className="col-span-2">
                  <label htmlFor="external-proxy-network" className="text-[11px] text-[#8a8a99] block mb-1.5">External Proxy Network</label>
                  <input
              ...
              <div className="col-span-2">
                <div className="flex items-center justify-between mb-1.5">
                  <label htmlFor="base-domain" className="text-[11px] text-[#8a8a99]">Base Domain (optional)</label>
                  <button
                    type="button"
              ...
              <div className="col-span-2 flex items-center gap-2">
                ...
                />
                <label htmlFor="auto-domains" className="text-[11px] text-[#8a8a99]">Auto-assign temporary domains for new services</label>
              </div>
            </div>
            ...
            {setupId && (
              <div className="border border-[rgba(255,255,255,0.08)] rounded-lg overflow-hidden">
                ...
                <div className="p-3 bg-[#161618] max-h-48 overflow-y-auto font-mono text-[10px] text-[#A0A0B0] space-y-1">
                  {provisionLogs.length === 0 && !provisionError && <div className="text-[#6b6b7b]">Waiting for logs...</div>}
